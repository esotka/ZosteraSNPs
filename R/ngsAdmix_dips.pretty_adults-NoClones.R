...
colorder <- list(
  ...
  c(1,3,5,4,2), #ks=5
  c(5,3,2,4,6,1), #ks=6
  c(6,3,7,5,1,2,4), #ks=7
  c(7,4,2,1,6,3,5,8), #ks=8
  c(4,8,3,9,6,5,2,1,7), #ks=9
  c(9,1,2,8,4,6,5,7,10,3)) #ks=10

pdf('output/ngsAdmix_dips.pretty-adults-NoClones.pdf',width=12,height=10)
par(mfrow=c(11,1),mar=c(0,0,0,0),xpd = TRUE)

for (k in 1:9)#length(ks))
{
  dat <- read.delim(paste("data/ngsadmix-noClones/",ks[k],".qopt",sep=""),sep=" ",header = F)
  dat <- dat[,-(dim(dat)[2])]
