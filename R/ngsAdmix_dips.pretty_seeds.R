...
colorder <- list(
  ...
  c(2,1,5,4,3), #ks=5
  c(3,6,5,2,4,1), #ks=6
  c(4,3,2,6,1,7,5), #ks=7
  c(3,7,8,6,2,1,4,5), #ks=8
  c(6,7,2,1,3,8,9,4,5), #ks=9
  c(6,1,3,2,9,8,5,10,7,4)) #ks=10
...
for (k in 1:9)#length(ks))
{
  ...
  fig <- barplot(t(dat),col=kcol[1:length(colorder[[k]])],space=0,border=NA,xlab="",ylab="",names.arg = rep("",nrow(dat)),horiz=F)#,main=substr(ks[k],1,3))
