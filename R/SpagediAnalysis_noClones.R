...
## 8 sites
rm(list=ls())
dat <- readLines("data/allSite.Spagedi.out_noClones_8categories.txt")
dat2 <- strsplit(dat,"\t")
ncat <- 8
...
mean.dist <- matrix(mean.dist,nrow=ncat,ncol=ncat,byrow=T)

## kinship
xbar <- c()
for (i in 3:10)
{xbar <- c(xbar,as.numeric(unlist(lapply(dat2[c(1:4,7,8,5,6)+8],"[[",i))))}
xbar <- matrix(xbar,nrow=ncat,ncol=ncat,byrow=T)

## lower CI
CI_inf <- c()
for (i in 3:10)
{CI_inf <- c(CI_inf,as.numeric(unlist(lapply(dat2[c(1:4,7,8,5,6)+16],"[[",i))))}
CI_inf <- matrix(CI_inf,nrow=ncat,ncol=ncat,byrow=T)

## upper CI
CI_sup <- c()
for (i in 3:10)
{CI_sup <- c(CI_sup,as.numeric(unlist(lapply(dat2[c(1:4,7,8,5,6)+24],"[[",i))))}
CI_sup <- matrix(CI_sup,nrow=ncat,ncol=ncat,byrow=T)
...
p <- matrix(p,nrow=ncat,ncol=ncat,byrow=T)
p <- ifelse(p<=0.05,"*","")

pdf("output/SpagediSGS_noClones_8categories.pdf",width=10,height=4)
