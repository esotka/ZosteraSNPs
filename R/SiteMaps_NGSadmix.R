...
a <- read.delim("data/k05run1.qopt",header=F,sep=" ")
...
xbar <- aggregate(a[,1:5],by=list(env$site.nice,env$adult.seed),mean)
# pie position
piex <- c(-70.91037,-70.84924,-70.78069,-70.67046)
piey <- c(42.34091,42.41549,42.46572,42.52)
segments(meta$lon[1:4],meta$lat[1:4],piex+0.025,piey)
segments(piex,piey,piex+0.05,piey)
# adults
for (j in 1:4)
{floating.pie(xpos=piex[j],ypos=piey[j],x=c(t(xbar[j,-(1:2)])),col=kcol,radius=0.02)}

# seeds
for (j in 1:4)
{floating.pie(xpos=piex[j]+0.05,ypos=piey[j],x=c(t(xbar[j+4,-(1:2)])),col=kcol,radius=0.02)}
text(x=piex,y=piey+0.027,"A",col="black")
